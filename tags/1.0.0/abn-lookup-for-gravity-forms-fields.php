 ...
 if (!class_exists( 'ITSG_GF_AbnLookup_Fields' )) {
    class ITSG_GF_AbnLookup_Fields {
							...
							if ( $key == $field['field_link_abnlookup'] ) {
								...
								if ( $field_hidden[$key] ) {
									...
									if ( 'IND' == $entityTypeCode ) {
										$entityName = $abn_details->businessEntity->legalName->familyName . ", " . $abn_details->businessEntity->legalName->givenName . " " .  $abn_details->businessEntity->legalName->otherGivenName;
									} else {
				...
				if ( strlen($numbersOnly) == 11 ) {
					...
					if (isset($abn_details->businessEntity)) {
						...
						if ($entityTypeCode == 'IND' ) {
							$entity = $abn_details->businessEntity->legalName->familyName . ", " . $abn_details->businessEntity->legalName->givenName . " " .$abn_details->businessEntity->legalName->otherGivenName;
						} else {
							$entity = $abn_details->businessEntity->mainName->organisationName;
						}
						$abn_details_message = $entityStatus .' - '.$entity;
					}
				} 
								...
								"use strict";
									var checkABR = function(data){
										var request = $.ajax({
											...
											data: data,
											beforeSend: function(){
												itsg_abnlookup_response.text( '<?php echo htmlentities($abnlookup_options['validation_message_loading']); ?>' );
												itsg_abnlookup_response.addClass( 'loading' );
												itsg_abnlookup_response.removeClass( 'error Active Cancelled' );
											},
												...
												if(typeof response !== 'undefined' ){
													try {
														...
														if (result["exception"] != undefined) {
															...
															itsg_abnlookup_response.addClass( 'error' );
															...
															if (entityTypeCode == 'IND' ) {
																var entityName = result['businessEntity']['legalName']['familyName'] + ", " + result['businessEntity']['legalName']['givenName'] + " " + result['businessEntity']['legalName']["otherGivenName"];
															} else {
																var entityName = result['businessEntity']['mainName']['organisationName'];
															}
															itsg_abnlookup_response.text( entityStatus + ' - ' + entityName );
															gform_abnlookup_entity_type_field_input.val( entityType );
															gform_abnlookup_entity_type_field_input.prop( 'readonly', true );
															gform_abnlookup_entity_status_field_input.val( entityStatus );
															gform_abnlookup_entity_status_field_input.prop( 'readonly', true );
															gform_abnlookup_entity_name_field_input.val( entityName );
															gform_abnlookup_entity_name_field_input.prop( 'readonly', true );
															gform_abnlookup_entity_postcode_field_input.val( entityPostcode );
															gform_abnlookup_entity_postcode_field_input.prop( 'readonly', true );
															gform_abnlookup_entity_state_field_input.val( entityState );
															gform_abnlookup_entity_state_field_input.prop( 'readonly', true );
															itsg_abnlookup_response.removeClass( 'error loading' );
									...
									if (gform_abnlookup_field.val().replace(/\D/g, '' ).length == 11 ) {
										// disable GST field that isnt currently used
										if ($( '.gform_abnlookup_entity_gst_field_<?php echo $field['id']; ?> input[value="Yes"]' ).is( ':checked' )) {
											$( '.gform_abnlookup_entity_gst_field_<?php echo $field['id']; ?> input[value="No"]' ).prop( 'disabled', true );
										} else if ($( '.gform_abnlookup_entity_gst_field_<?php echo $field['id']; ?> input[value="No"]' ).is( ':checked' )) {
											$( '.gform_abnlookup_entity_gst_field_<?php echo $field['id']; ?> input[value="Yes"]' ).prop( 'disabled', true );
										}
									...
									gform_abnlookup_field.change(function() {
										...
										request = checkABR(data);
											...
											itsg_abnlookup_response.text( "<?php echo htmlentities($abnlookup_options['validation_message_11_char']); ?>" );
											gform_abnlookup_entity_gst_field_yes.prop( 'disabled', false );
											gform_abnlookup_entity_gst_field_no.prop( 'disabled', false );
											gform_abnlookup_entity_gst_field_yes.prop( 'checked', false );
											gform_abnlookup_entity_gst_field_no.prop( 'checked', false );
		...
		public static function abnlookup_css_class( $classes, $field, $form ) {
			if ( 'abn' == self::is_abnlookup_field($field) ) {
				$classes .= " gform_abnlookup_field_".rgar($field, 'id' );
			} elseif ( 'abnlookup_entity_gst' == self::is_abnlookup_field($field) ) {
