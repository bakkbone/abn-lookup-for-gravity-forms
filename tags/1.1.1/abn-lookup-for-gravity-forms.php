...
if (!class_exists( 'ITSG_GF_AbnLookup' )) {
	class ITSG_GF_AbnLookup {
		...
		function register_actions() {
            if ( self::is_gravityforms_installed() && extension_loaded('soap') ) {
				// start the plugin
		...
		public static function admin_warnings() {
			...
			if ( !extension_loaded('soap') ) {
				...
				<div class="error">
					...
					<p>
						<?php _e( 'The plugin ', self::$slug); ?><strong><?php echo self::$name; ?></strong> <?php _e( 'requires the PHP SOAP extension to be installed on the web server hosting the WordPress installation.', self::$slug) ?>
					</p>
					<p>
						<?php _e( 'This plugin will not function until SOAP is installed and enabled.', self::$slug); ?>
					</p>
					<p>
						<?php _e( 'Contact your hosting provider if you do not know how to install the PHP SOAP extension.', self::$slug); ?>
					</p>
				</div>
				<?php
			} elseif ( !self::is_gravityforms_installed() ) {
				?>
				<div class="error">
					<h3><?php _e( 'Warning', self::$slug); ?></h3>
					<p>
            ...
            return array(
                array(
                    ...
                    "fields" => array(
						...
						array(
                            "label"   => __( 'Error comminicating message', 'itsg_gf_abnlookup' ),
							"name"    => "validation_message_error_communicating",
                            "tooltip" => __( 'This message is displayed to the user when the ABN Lookup script has failed to communicate with the Australian Business Register more than three times.', 'itsg_gf_abnlookup' ),
                            "type"    => "textarea",
                            "class"   => "medium",
