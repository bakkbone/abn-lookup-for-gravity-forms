 ...
 if ( !class_exists( 'ITSG_GF_AbnLookup_Fields' ) ) {
    class ITSG_GF_AbnLookup_Fields {
				...
				foreach( self::$form['fields'] as &$field )  {
					...
					?>
						...
						function itsg_gf_abnlookup_function_<?php echo $field['id']; ?>(self){
							(function( $ ) {
								"use strict";
									...
									var gform_abnlookup_field = $( '.gform_abnlookup_field_<?php echo $field['id']; ?> input[type="text"]' );
									...
									var itsg_abnlookup_checkabn_button = $( '.itsg_abnlookup_checkabn_<?php echo $field['id']; ?>' );
						...
						jQuery(document).bind( 'gform_post_render gform_post_conditional_logic', function( $ ) {
							...
							jQuery( "#input_<?php echo self::$form['id']; ?>_<?php echo $field['id']; ?>" ).unbind( 'keydown' ).keydown(function( event ) {
			...
			if ( 25 == $position ) {
				...
				<li class="abnlookup_validate_field_setting field_setting" style="display:list-item;">
					<label for="field_validate_abnlookup" class="inline">
							<?php _e( "Validate ABN Lookup field", "abn-lookup-for-gravity-forms" ); ?>
						</label>
		...
		public static function field_tooltips( $tooltips ){
			...
			$tooltips["form_field_validate_abnlookup"] = "<h6>".__( "Validate ABN Lookup field", "abn-lookup-for-gravity-forms" )."</h6>".__( "Choose the level of validation required for the ABN Lookup field.", "abn-lookup-for-gravity-forms" );
