...
if ( !class_exists( 'ITSG_GF_AbnLookup' ) ) {
	class ITSG_GF_AbnLookup {
		...
		public static function do_abnlookup( $abn ) {
			...
			$result = simplexml_load_string($result)->response;
		...
		public static function get_options(){
			$defaults = array(
				'guid' => '',
				'includecss' => true,
				'validation_message_not_valid' => 'The ABN provided is not valid. Check the number entered and try again, or use the <a href="https://abr.business.gov.au/" target="_blank">ABN Lookup website</a> to confirm for your ABN.',
		...
		public static function admin_warnings() {
			$abnlookup_options = self::get_options();
			$abnlookup_settings_url = '<a href="' . admin_url( 'admin.php?page=gf_settings&subview=itsg_gf_abnlookup_settings' ) . '" title="' . esc_attr( __( 'View ABN Lookup for Gravity Forms  Settings', 'abn-lookup-for-gravity-forms' ) ) . '">' . __( 'ABN Lookup for Gravity Forms Settings', 'abn-lookup-for-gravity-forms' ) . '</a>';
			if ( !self::is_gravityforms_installed() ) {
				$gravityforms_url = '<a target="_blank" href="https://www.e-junkie.com/ecom/gb.php?cl=54585&c=ib&aff=299380" title="' . esc_attr( __( 'Gravity Forms website', 'abn-lookup-for-gravity-forms' ) ) . '">' . __( 'download the latest version', 'abn-lookup-for-gravity-forms' ) . '</a>';
				...
				$html = sprintf(
					'<div class="error"><h3>%s</h3><p>%s</p><p>%s</p></div>',
						...
						sprintf ( __( 'Please %s of Gravity Forms and try again.', 'abn-lookup-for-gravity-forms' ), $gravityforms_url )
				);
				echo $html;
			} elseif ( '' == $abnlookup_options['guid'] ) {
				$abr_url = '<a target="_blank" href="http://abr.business.gov.au/webservices.aspx" title="' . esc_attr( __( 'Australian Business Register website', 'abn-lookup-for-gravity-forms' ) ) . '">' . __( 'web services registration', 'abn-lookup-for-gravity-forms' ) . '</a>';
				$html = sprintf(
					'<div class="error"><h3>%s</h3><p>%s</p><p>%s</p><p>%s</p></div>',
						__( 'Warning', 'abn-lookup-for-gravity-forms' ),
						sprintf ( __( 'The plugin %s requiresa GUID to communicate with the Australian Business Register.', 'abn-lookup-for-gravity-forms' ), '<strong>'.self::$name.'</strong>' ),
						sprintf ( __( 'To receive a GUID see %s on the Australian Business Register website.', 'abn-lookup-for-gravity-forms' ), $abr_url ),
						sprintf ( __( 'Once you have a GUID you will need to enter it in the %s page.', 'abn-lookup-for-gravity-forms' ), $abnlookup_settings_url )
				);
				echo $html;
			}
		} // END admin_warnings
...
if ( class_exists( "GFForms" ) ) {
	...
	class ITSG_GF_AbnLookup_Settings extends GFAddOn {
                ...
                array(
                    ...
                    "fields" => array(
                        array(
							...
							"name"    => "guid",
                            "tooltip" => __( 'To receive a GUID see <a target="_blank" href="http://abr.business.gov.au/webservices.aspx">web services registration</a> on the Australian Business Register website.', 'abn-lookup-for-gravity-forms' ),
                            "type"    => "guid"
                        ),
						array(
		...
		public function settings_guid(){
				...
				?><div>
                <p><?php _e( 'To receive a GUID see <a target="_blank" href="http://abr.business.gov.au/webservices.aspx">web services registration</a> on the Australian Business Register website.', 'abn-lookup-for-gravity-forms' ) ?></p>
            </div><?php
        } // END settings_guid
